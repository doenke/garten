...
class WfoTaxonomyExtractionTest(unittest.TestCase):
    ...
    def test_extract_wfo_taxon_slug_prefers_search_result_links(self):
        ...
        self.assertEqual(views._extract_wfo_taxon_slug(page_html), 'wfo-0000000002')

    def test_wfo_taxonomy_id_returns_first_search_result_slug(self):
        page_html = """
            <div class="not-result">
                <a href="/taxon/wfo-4000029286">A different plant name</a>
            </div>
        ...
        with patch.object(views, '_search_page_html', return_value=page_html):
            self.assertEqual(views._wfo_taxonomy_id('Requested Name', {}), 'wfo-0000000002')
